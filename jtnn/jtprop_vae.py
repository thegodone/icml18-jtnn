...
class JTPropVAE(nn.Module):
    ...
    def forward(self, mol_batch, beta=0):
        ...
        epsilon = torch.randn(batch_size, self.latent_size / 2).to(self.device)
        ...
        prop_label = torch.Tensor(prop_batch).to(self.device)
    ...
    def assm(self, mol_batch, mol_vec, tree_mess):
        ...
        batch_idx = torch.tensor(batch_idx,dtype=torch.long).to(self.device)
    ...
    def stereo(self, mol_batch, mol_vec):
        ...
        if len(labels) == 0: 
            return torch.Tensor(0).to(self.device), 1.0
    ...
    def reconstruct(self, smiles, prob_decode=False):
        ...
        epsilon = torch.randn(1, self.latent_size / 2).to(self.device)
        tree_vec = tree_mean + torch.exp(tree_log_var / 2) * epsilon
        epsilon = torch.randn(1, self.latent_size / 2).to(self.device)
        mol_vec = mol_mean + torch.exp(mol_log_var / 2) * epsilon
    ...
    def sample_prior(self, prob_decode=False):
        tree_vec = torch.randn(1, self.latent_size / 2).to(self.device)
        mol_vec = torch.randn(1, self.latent_size / 2).to(self.device)
        return self.decode(tree_vec, mol_vec, prob_decode)
    ...
    def optimize(self, smiles, sim_cutoff, lr=2.0, num_iter=20):
        ...
        cur_vec = mean.data.to(self.device).requires_grad_()
        ...
        for step in xrange(num_iter):
            ...
            cur_vec = cur_vec.to(self.device).requires_grad_()
